...
ggplot(plot_data_16_years, aes(x = Date, group = 1)) +
  geom_line(aes(y = Main_Series, color = "Main Series"), size = 1) +
  geom_line(aes(y = Long_Term_Trend, color = "Long-Term Trend"), size = 1) +
  geom_line(aes(y = Gap_16_Years, color = "Gap (16 Years)"), size = 1) +
...
# Create a new dataframe with Date in yyyy-mm format and Loan-to-deposit ratio
data_LDR <- data.frame(
  Date = dates,
